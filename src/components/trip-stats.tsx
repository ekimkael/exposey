...
/**
 * One row entry in the stats grid.
 * `iconKey` must be a known key from the ICONS map.
 */
type Stat = {
  label: string;
  count: number;
...
/** Props for TripStats. */
type Props = {
  /** Array of stat entries to render as a 4-column grid. */
  stats: readonly Stat[];
  /** Optional heading above the grid. Defaults to "Trip includes". */
  heading?: string;
};

// ─── Stat icons (Tabler filled, muted grey) ────────────────────────────────
...
const ICONS: Record<StatIconKey, () => React.ReactElement> = {
  hotel:      HotelIcon,
  car:        CarIcon,
...
export default function TripStats({ stats, heading = 'Trip includes' }: Props) {
  return (
        ...
        {stats.map((stat) => {
          ...
          return (
            <View key={stat.iconKey} style={s.cell}>
              <Icon />
              <Text style={s.count}>{String(stat.count).padStart(2, '0')}</Text>
